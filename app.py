...
STRAVA_REDIRECT_URI = os.environ.get('STRAVA_REDIRECT_URI')  # e.g., 'https://yourdomain.com/strava/callback'
...
class User(db.Model):
    ...
    user_races = db.relationship('UserRace', backref='user', lazy=True)
...
class Race(db.Model):
    ...
    start_latitude = db.Column(Float, nullable=False)
    start_longitude = db.Column(Float, nullable=False)
    mid_latitude = db.Column(Float, nullable=False)
    mid_longitude = db.Column(Float, nullable=False)
    end_latitude = db.Column(Float, nullable=False)
    end_longitude = db.Column(Float, nullable=False)
...
class UserRace(db.Model):
    ...
    activity_id = db.Column(db.String(100), nullable=False)  # Strava Activity ID

    user = db.relationship('User', backref=db.backref('user_races', lazy=True))
    race = db.relationship('Race', backref=db.backref('user_races', lazy=True))

    def __repr__(self):
        return f"<UserRace User {self.user_id} Race {self.race_id}>"
...
def refresh_strava_token(user):
    ...
    if user.strava_refresh_token and user.strava_expires_at:
        if datetime.utcnow() > user.strava_expires_at:
            ...
            user.strava_expires_at = datetime.fromtimestamp(data.get('expires_at'))
            db.session.commit()
...
def is_within_radius(point1, point2, radius_km=0.5):
    """
    Check if two points are within the specified radius in kilometers.
    ...
    Returns:
    - Boolean indicating if points are within the radius
    """
    distance = geodesic(point1, point2).kilometers
    return distance <= radius_km


def get_activity_gps_data(access_token, activity_id):
    """
    Fetch the GPS data (stream) for a specific activity.
    ...
    Returns:
    - Dictionary containing GPS data or None if failed
    """
    headers = {
        'Authorization': f'Bearer {access_token}'
    }
    # Fetch activity details to get polyline or individual streams
    activity_response = requests.get(f'https://www.strava.com/api/v3/activities/{activity_id}', headers=headers)

    if activity_response.status_code != 200:
        logging.error(f"Failed to fetch activity {activity_id}: {activity_response.text}")
        return None

    activity_data = activity_response.json()

    # Option 1: Use polyline
    polyline = activity_data.get('map', {}).get('summary_polyline')
    if polyline:
        return {'polyline': polyline}

    # Option 2: Fetch individual streams
    streams_response = requests.get(f'https://www.strava.com/api/v3/activities/{activity_id}/streams', headers=headers)
    if streams_response.status_code != 200:
        logging.error(f"Failed to fetch streams for activity {activity_id}: {streams_response.text}")
        return None
...
def decode_polyline(encoded_polyline):
    """
    Decode an encoded polyline into a list of (latitude, longitude) tuples.
    ...
    try:
        ...
        logging.error(f"Failed to decode polyline: {e}")
        return []
...
def calculate_achievements(df):
    ...
    categories_info = {
        '💲 Distance': 'Earn coins based on the distance you\'ve run or ridden. The more you cover, the more coins you earn.',
        '💰 Elevation': 'Earn coins by conquering elevation challenges. Gain coins as you ascend higher.',
        '🧈 Consistency': 'Earn coins by maintaining consistent activity. Stay active and accumulate more coins.',
        '💎 KCal': 'Burn calories and earn coins. The more calories you burn, the more coins you collect.',
        '👑 Streaks': 'Maintain streaks of consecutive days or weeks to earn additional coins.'
    }
    ...
    # Define new Achievements with specified emojis and assign to appropriate categories
    new_achievements = [
        {
    ...
    for achievement in new_achievements:
        category_name = achievement.pop('category')
        activity_type = achievement.pop('type')
        for category in achievements['categories']:
            if category['name'] == category_name:
                category['achievements'][activity_type].append(achievement)
                break

    # ------------------ Additional Achievements (Medals) ------------------
    ...
    for badge in additional_achievements:
        category_name = badge.pop('category')
        activity_type = badge.pop('type')
        for category in achievements['categories']:
            if category['name'] == category_name:
                category['achievements'][activity_type].append(badge)
                break
    ...
    # ------------------ New Medals ------------------
    # Define new Medals with their respective emojis
    new_medals = [
        {
            'name': 'Steep Climber',
            'emoji': '🧗‍♀️',
...
def calculate_stats(df):
    ...
    stats = {
        ...
        'bpm': float(round(df['Average_heart_beat'].mean(), 1)),  # in bpm
...
def calculate_achievements(df):
    ...
    for activity_type, info in distance_badges.items():
        ...
        for idx, threshold in enumerate(thresholds):
            ...
            if (activity_type == 'Run' and threshold >= 50) or (activity_type == 'Ride' and threshold >= 300):
                # Weekly threshold
                df_sorted['Week'] = df_sorted['Activity_Date'].dt.to_period('W')
                weekly_distance = df_sorted[df_sorted['Activity_Type'] == activity_type].groupby('Week')['Distance_km'].sum()
                count = int((weekly_distance >= threshold).sum())
            ...
            category_name = '💲 Distance'  # All distance achievements go under '💲 Distance'
            for category in achievements['categories']:
                if category['name'] == category_name:
                    category['achievements'][activity_type].append({
                        'name': name,
    ...
    # ========== Streaks of Months ==========
    months_sorted = sorted(months_unique, key=lambda x: x.start_time)
    max_month_streak = 1
    current_month_streak = 1
    for i in range(1, len(months_sorted)):
        previous_month = months_sorted[i-1]
        current_month = months_sorted[i]
        expected_next_month = previous_month + 1
        if current_month == expected_next_month and \
           (monthly_days.get(current_month, 0) == (current_month.end_time - current_month.start_time).days + 1):
            current_month_streak += 1
            if current_month_streak > max_month_streak:
                max_month_streak = current_month_streak
        else:
            current_month_streak = 1

    # Assign Streaks to '👑 Streaks' category
    category_name = '👑 Streaks'
    streak_thresholds = [2, 6, 12]  # 2 months, 6 months, 12 months
    streak_emojis = ['🧈', '💎', '👑']
    for category in achievements['categories']:
        if category['name'] == category_name:
            for threshold, emoji in zip(streak_thresholds, streak_emojis):
                if max_month_streak >= threshold:
                    category['achievements']['Run'].append({
                        'name': f'{threshold}-Month Streak',
                        'emoji': emoji,
                        'description': f'Logged activities for {threshold} consecutive months',
                        'count': 1  # Assuming one streak per threshold
                    })
            break
    ...
    new_achievements = [
        {
            'name': 'Everesting Ascent',
            'emoji': '💎',
            'description': 'Logged an activity with Everesting ascention (8848m)',
            'count': int(df[df['Elevation_Gain'] >= 8848].shape[0]),
            'category': '💰 Elevation',
            'type': 'Run'  # Assuming Everesting is a Run activity
        },
...
def strava_callback():
    """Handle the callback from Strava and exchange code for access token."""
    ...
    user = User.query.filter_by(strava_link=session.get('strava_link')).first()
    if user:
        user.strava_access_token = access_token
        user.strava_refresh_token = refresh_token
        user.strava_expires_at = datetime.fromtimestamp(expires_at)
        db.session.commit()
